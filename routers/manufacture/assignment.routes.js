...
router.get("/", async (req, res, next) => {
  try {
    ...
    console.log(error);
    res.status(500).json({ error: "Lỗi khi lấy danh sách phân công sản xuất." });
  }
});
...
router.post("/", async (req, res, next) => {
  try {
    ...
    res.status(200).json({ message: "Phân công sản xuất đã được thêm." });
  } catch (error) {
    console.log(error);
...
router.put("/", async (req, res, next) => {
  try {
    ...
    res.status(200).json({ message: "Cập nhật phân công sản xuất thành công" });
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: "Lỗi khi cập nhật phân công sản xuất" });
  }
});
...
router.delete("/", async (req, res, next) => {
  try {
    ...
    res.status(200).json({ message: "Xóa phân công sản xuất thành công." });
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: "Lỗi xóa phân công sản xuất." });
  }
});
